...
def attach_hooks(config, model, hooks, ignore=""):
    for n, m in model.named_modules():
        # if n in arch_config["targets"]:
        if isinstance(m, (nn.Conv2d, nn.BatchNorm2d, nn.LayerNorm, nn.Linear)):
            if n != ignore:
...
class Hook:
    ...
    def hook_fn(self, module: torch.nn.Module, input: torch.Tensor, output: torch.Tensor) -> None:
        ...
        # TODO sort this mess
        if isinstance(self.module, (nn.Linear, nn.LayerNorm)):
            if len(output.shape) > 2:
                reshaped_output = output.view(output.shape[0], output.shape[-1], -1)
            else:
                reshaped_output = output.view(output.shape[0], output.shape[1])
        else:
            reshaped_output = output.view((output.shape[0], output.shape[1], -1))

        if self.config.mask_mode == "per-sample":
            reshaped_output = reshaped_output.mean(dim=2)
...
def cosine_similarity(x1, x2, dim, eps=1e-8):
    ...
    x1_normalized[torch.isnan(x1_normalized)] = 0.
    x1_normalized[torch.isinf(x1_normalized)] = 0.
    
    x2_normalized = x2.div(x2_norm)
    x2_normalized[torch.isnan(x2_normalized)] = 0.
    x2_normalized[torch.isinf(x2_normalized)] = 0.
